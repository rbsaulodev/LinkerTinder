...
class CandidatoDAO implements ICandidatoDAO {
    ...
    CandidatoDAO(IConexao conexao, ICompetenciaDAO competenciaDAO) {
    ...
    Candidato inserir(Candidato candidato) {
        ...
        try {
            ...
            if (candidato.endereco?.id) stmt.setInt(8, candidato.endereco.id)
        ...
        println "Candidato '${candidato.nome}' inserido com ID ${candidato.id}."
        return candidato
    }
    ...
    List<Candidato> listarTodos() {
        String sql = """
            ...
            FROM candidatos c LEFT JOIN enderecos e ON e.id = c.endereco_id ORDER BY c.nome
        """
    ...
    Candidato buscarPorId(int id) {
        String sql = """
            SELECT c.*, e.id as eid, e.cep, e.logradouro, e.numero, e.complemento,
                   e.bairro, e.cidade, e.estado, e.pais
            FROM candidatos c LEFT JOIN enderecos e ON e.id = c.endereco_id WHERE c.id = ?
        """
        Connection con = conexao.obterConexao()
        try {
    ...
    boolean atualizar(Candidato candidato) {
        String sql = """
            UPDATE candidatos SET nome=?, sobrenome=?, email=?, cpf=?, data_nasc=?, descricao=?
            WHERE id=?
        """
        Connection con = conexao.obterConexao()
        try {
    ...
    boolean deletar(int id) {
        Connection con = conexao.obterConexao()
        try {
            PreparedStatement stmt = con.prepareStatement("DELETE FROM candidatos WHERE id=?")
            stmt.setInt(1, id)
            boolean ok = stmt.executeUpdate() > 0
    ...
    private Candidato mapear(ResultSet rs) {
        ...
        new Candidato(id: rs.getInt('id'), nome: rs.getString('nome'),
                sobrenome: rs.getString('sobrenome'), email: rs.getString('email'),
                cpf: rs.getString('cpf'), dataNasc: rs.getDate('data_nasc').toLocalDate(),
